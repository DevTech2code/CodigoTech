...
let KeyboardService = class KeyboardService {
    ...
    async create(dto) {
        try {
            ...
            (0, prisma_error_util_1.handlePrismaError)(error, 'Teclado');
        }
    }
    async findAll() {
    ...
    async findOne(id) {
        ...
        if (!keyboard)
            throw new common_1.NotFoundException(`Teclado con ID ${id} no encontrado`);
        return keyboard;
    }
    async update(id, dto) {
        try {
            ...
            (0, prisma_error_util_1.handlePrismaError)(error, 'Teclado', id);
        }
    }
    async remove(id) {
        try {
