...
let SupportService = class SupportService {
    ...
    async create(dto) {
        try {
            ...
            (0, prisma_error_util_1.handlePrismaError)(error, 'Support');
        }
    }
    async findAll() {
    ...
    async findOne(id) {
        ...
        if (!support)
            throw new common_1.NotFoundException(`Soporte con ID ${id} no encontrado`);
        return support;
    }
    async update(id, dto) {
        try {
            ...
            (0, prisma_error_util_1.handlePrismaError)(error, 'Support', id);
        }
    }
    async remove(id) {
        try {
